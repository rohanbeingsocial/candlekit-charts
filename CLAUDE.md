...
- **`npm run build:site`** (`scripts/build-site.mjs`) builds every example into
  ...
  `scripts/site-assets/*` (incl. `preview.gif`) at the site root. Override base
  for a local root preview: `SITE_BASE=/ npm run build:site`. `site/` is
  gitignored.
...
- **Hero GIF** (`scripts/site-assets/preview.gif`) is regenerated by
  `scripts/capture-demo.mjs` — Playwright drives the running replay demo (system
  Chrome via `channel:"chrome"`), screenshots frames, encodes with `gifenc`
  (no ffmpeg). Run: `cd examples/replay && npm run dev`, then in the repo root
  with `playwright-core gifenc pngjs` resolvable, `node scripts/capture-demo.mjs`
  (env: `SPEED PREROLL_MS FRAMES DELAY_MS WIDTH HEIGHT`). README references the
  GIF by raw GitHub URL so it renders on github.com + npm once public.
