...
- **Lab (experimental)** — pattern-similarity analytics over your own bars.
  **Echoes** ("market déjà vu") finds historical windows that look like the
  recent price action, bands them, and projects the median of what happened
  next; **Sketch Search** matches a freehand shape you draw against history.
  Pure, dependency-free math + optional plugins and a React panel.
...
<p align="center">
  <img src="https://raw.githubusercontent.com/rohanbeingsocial/candlekit-charts/main/scripts/site-assets/lab.gif" alt="Echoes highlights historical windows that look like the recent price action and projects the median of what happened next; Sketch Search matches a freehand shape against history" width="820" />
  <br />
  <em>Echoes — similar past windows + median projection · Sketch Search — draw a shape, find look-alikes</em>
</p>

Two experimental pattern-similarity tools, computed on **z-normalized** closes
(shape, not price level), so look-alikes are found across regimes:

- **Echoes** — takes the last *N* closes as a query, finds the most similar
  non-overlapping windows in history, bands them on the chart, and projects the
  **median of what happened next** forward from the last bar.
- **Sketch Search** — draw a freehand shape across the chart; it is resampled
  and matched against history the same way.

The math (`zNormalize`, `findSimilar`, `buildEchoScan`, `resampleStroke`) is
pure and dependency-free; the chart plugins (`SketchSearchController`,
`EchoesController`) and the React `<EchoesPanel>` / `<SketchSearchButton>` are
optional. See **[Lab Examples](#lab-examples)** below, the full
[Lab guide](docs/lab.md), and [examples/lab](examples/lab) for a runnable demo.

> Try it live in [examples/lab](examples/lab) (`cd examples/lab && npm i && npm run dev`).
...
<ChartView data={bars} sketch echoes={{ windowLen: 30, horizon: 30, k: 8 }}>
  <SketchSearchButton />  {/* toggle, then drag a shape on the chart */}
...
chart.use(echoes);
echoes.subscribe((scan) => scan && console.log(scan.stats)); // { count, upCount, medianEndPct, ... }
echoes.run();
...
const matches = findSimilar(closes, closes.slice(-30), { k: 5 }); // [{ startIndex, endIndex, distance }]
const scan = buildEchoScan(bars, 30, 30, 8);                      // window/horizon/k → null if too little history
```
...
| `EchoesController`, `SketchSearchController` | class | Lab plugins — Echoes (déjà vu) + Sketch Search. |
| `findSimilar`, `buildEchoScan`, `zNormalize`, `resampleStroke` | fn | Pure pattern-similarity math (no chart). |
| `ChartView` *(/react)* | component | Declarative chart + plugin host. |
| `EchoesPanel`, `SketchSearchButton` *(/react)* | component | Lab results panel + sketch toggle. |
